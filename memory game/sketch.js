...
let grid; //empty grid
let completedGrid;   //answer key
let rows = 4;
let cols = 4;
let currentlySelectedCells = []; // whats currently clicked
const EMPTY = 100;
let currentlyAnimating = false;  // whether or not its staying or going back to greyt
let state = "gameplay";
...
function createCompletedGrid() {
  let colorList = ["blue", "red", "green", "yellow", "teal", "black", "purple", "white"];
  let pairs = [];
  for (let thisColor of colorList) {
    pairs.push(thisColor); 
    pairs.push(thisColor);
  }
  shuffle(pairs, true); // changes value of pairs to be randomized (SUPER USEFUL)
